...
class RNNPy2DSobelX(RNNBatchPy):
...
def imageTestPlot(rnn, im, convRes, compRes, result, resDir, filePrefix):
    ...
    plt.axis('off')
    plt.title('Convolved Sobel', fontsize=20)
    plt.subplot(143)
    plt.imshow(compRes, cmap=plt.cm.gray)
    plt.axis('off')
    plt.title('Computed Sobel', fontsize=20)
    plt.subplot(144)
...
# Create RNN
rnnPyTh = RNNPy2DSobelX(nh, nin, nout, nbatches)
...
filePrefix = 'rnn_2DSobelX'
imageTestPlot(rnnPyTh, im, sx, sx, res, resDir, filePrefix)
...
for imname in imList:
    ...
    sx = ndimage.sobel(im, axis=1, mode='constant')
    res, sm = rnnPyTh.imageTest(im, M)
    filePrefix = 'rnn_2DSobelX_' + imname.split(".")[0]
    imageTestPlot(rnnPyTh, im, sx, sx, res, resDir, filePrefix)
