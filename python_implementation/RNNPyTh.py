...
class RNNPyTh(object):
    ...
    def __init__(self, nh, nin, nout):
        ...
        self.th_error = 0.5 * ((self.th_y - self.th_t) ** 2).sum()
        ...
        self.th_fn = theano.function(
            [self.th_x, self.th_h0, self.th_t,
             self.th_W_xh, self.th_W_hh, self.th_W_hy],
            [self.th_gW_xh, self.th_gW_hh, self.th_gW_hy, self.th_error])
    ...
    def sgdStep(self, x, t, h0, m=0.001, errorThresh=0.01):
        ...
        th_dE_xh, th_dE_hh, th_dE_hy, th_err = \
            self.th_fn(x, h0, t,
    ...
    def trainNetworkSGD(self, iters, dataLen, step, nepoch, dataDir, filePrefix):
        ...
        for epoch in xrange(nepoch):
            ...
            for idx in xrange(iters):
                ...
                # gradient, using SGD
                loss = self.sgdStep(x, t, h0)
                totalLossTheano += loss
            ...
            print "%s: Epoch = %d, numpy loss = %f, theano loss = %f" \
                  % (curTime, epoch, totalLossNumpy, totalLossTheano)

        curTime = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
        rnnName = dataDir + '/' + filePrefix + '-%d-%d-%d-%s.npz' % \
                                         (self.nh, self.nin, self.nout, curTime)
        self.saveNetwork(rnnName)
        return lossesNumpy, lossesTheano
