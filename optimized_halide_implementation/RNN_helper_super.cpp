...
int main(){
    ...
    Func bprop_dEdh_in;
    bprop_dEdh_in(i,j) = (sum(dEdy_in(ry,j) * Why(ry,i)) + sum(dEdh_in_tp1(rh,j) * Whh(rh,i)))*(1-h_t(i,j)*h_t(i,j));
    PARALLEL_MATRIX(bprop_dEdh_in);
    bprop_dEdh_in.compile_to_file("bprop_dEdh_in",{t, h_t, dEdh_in_tp1, dEdy_in, Whh, Why});

    /* Calculate Gradients Ghy */
    Func bprop_Ghy;
    bprop_Ghy(i,j) = Ghy(i,j) + sum(h_t(j,rb) * dEdy_in(i,rb));
    PARALLEL_MATRIX(bprop_Ghy);
    bprop_Ghy.compile_to_file("bprop_Ghy",{t, h_t, dEdy_in, Ghy});

    /* Calculate Gradients Ghh */
    Func bprop_Ghh; 
    bprop_Ghh(i,j) = Ghh(i,j) + sum(h_tm1(j,rb) * dEdh_in(i,rb));
